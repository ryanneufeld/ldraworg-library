...
class LastDayDownloadZipController extends Controller
{
    public function __invoke()
    {
        ...
        Part::whereHas('events', fn (Builder $q) => $q->where('created_at', '>=', now()->subDay())->whereHas('part_event_type', fn (Builder $qu) => $qu->whereIn('slug', ['submit', 'rename', 'edit'])
        )
        )
            ->each(fn (Part $part) => $zip->addFromString($part->filename, $part->get())
            );
        $zip->close();
