...
class LDrawFileValidate
{
   ...
   * @return array
   */
  public static function validAuthor(string $text): array
  {
   ...
   * @return array
   */
  public static function validCategory(string $text): array
  {
    ...
    $results = [];
    if (!empty($pt) && ($pt->type == 'Part' || $pt->type == 'Shortcut') && !PartCheck::checkCategory($text)) {
  ...
  public static function validDescription($text)
  {
    ...
    if (! PartCheck::checkDescription($text)) {
      ...
      in_array($lastword, ['Pattern', '(Obsolete)', 'Work)'])) {
  ...
  public static function validLines($text)
  {
    ...
    foreach ($text as $index => $line) {
      ...
      } elseif (! empty($line) && $index > $headerend && $line[0] === 0 && ! in_array(strtok(mb_substr($line, 1), ' '), config('ldraw.allowed_metas.body'), true)) {
  ...
  public static function validPartType($text, $input_pt = null)
  {
    ...
    if (isset($input_pt)) {
    $form_type = PartType::find($input_pt);
    }
  ...
  public static function validSubmitAuthor($text, $user_id)
  {
    ...
    if (isset($fuser) && $fuser->id === $user_id) {
    return;
    }
